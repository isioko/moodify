...
class ViewController: UIViewController, SPTSessionManagerDelegate, SPTAppRemoteDelegate, SPTAppRemotePlayerStateDelegate {
    ...
    @IBAction func clickRefreshTokenButton(_ sender: UIButton) {
        getToken()
    }
    func getToken(){
        let composed_url = "https://accounts.spotify.com/authorize?client_id="+SpotifyClientID+"&response_type=code&redirect_uri="+"moodify://spotify-login-callback" 
        let url = URL(string:composed_url)
    ...
    @IBAction func getTopArtists(_ sender: UIButton) {
       getArtistsTop()
    }
...
        request.addValue("Bearer BQCSPr1wHKyNNC-VBVrrCDeLYt9ygwAdRUqNIzK1y4RfKCQlOLN8JmnpIOMBdK8AoD2DVBGr9RzpWArRp6HMke1vTDpdwEOzDWxplpY95KorlRlBc0HUQyHuMMs3dIm_iiQaJ3uqnECe48Jx8WPjVAfDgzypXrJVTv6ATKgxYZyMkf1e9Sf__wtYEPSQY5wguqD_DWXcSzNT8ke6oZqqRM09A1fE7-nrvdRYKYkQtrDT0F0PDwzhIELIvpIvxOJ94w-uOM2UCrNRxwwIww", forHTTPHeaderField: "Authorization")
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            guard error == nil else {
                print(error!)
                return
    ...
    lazy var configuration: SPTConfiguration = {
        ...
        configuration.tokenSwapURL = URL(string: "http://moodify18.herokuapp.com/api/swap")
        configuration.tokenRefreshURL = URL(string: "http://moodify18.herokuapp.com/api/refresh")
